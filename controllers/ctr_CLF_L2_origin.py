...
class Controller_CLF2Orig(Controller): 
    # BUG goes in a straight line and stops when tangent to origin.
    # CLF is just l2norm, so maybe theta being in that is making it weird.
    def __init__(self, dynsys:ControlAffineSys) -> None:
        super().__init__(dynsys=dynsys)
        #self.dynsys.modify_ubounds(np.tile(np.array([[-np.inf,np.inf]]),(self.dynsys.uDims,1)))
    ...
    def clf(self, x:np.ndarray) -> Tuple[float,np.ndarray]:
        return np.linalg.norm(x)**2 / 2 , x.T
    ...
    def makeProblem_CLFQP(self):
        ...
        c = 1
        constraints.append(self.LfV_param + self.LgV_param@self.u_var + .005*c*self.V_param - self.r_var <= 0)
